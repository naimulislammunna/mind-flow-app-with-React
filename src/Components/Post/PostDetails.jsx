...
import DwonVote from "../Vote/DwonVote";
...
const PostDetails = () => {
    ...
    const handleUpVote = async () => {
        await axiosSecure.patch(`/up-vote/${data._id}`)
            .then(res => {
    ...
    const handleDwonVote = async () => {
        await axiosSecure.patch(`/dwon-vote/${data._id}`)
            .then(res => {
                if (res.data.modifiedCount) {
                    refetch()
                }
    ...
    const submitComment = async () => {
        ...
        await axiosSecure.patch(`/comment/${data._id}`, { comment: value })
        .then(res => {
            if(res.data.modifiedCount){
                refetch()
            }
    ...
    return (
        <div>
            <div className=" mx-auto my-6 max-w-[650px] rounded-lg bg-white shadow-lg dark:bg-[#18181B] border border-gray-500">
                <div className="flex items-center justify-between gap-10 px-4 py-4">
                    {/* Avatar image  */}
                    <div className="flex items-center gap-3">
                        <img width={100} height={100} className="h-16 w-16 rounded-full bg-black/40 object-cover" src={data?.photoURL} alt="card navigate ui" />
                        <div className="flex flex-col">
                            <h2 className="text-xl font-semibold">{data?.displayName}</h2>
                            <p className="text-gray-400">{data?.time?.slice(0, 10)}</p>
                        </div>
                    </div>
                </div>
                {/* Post content */}
                <div className="mt-3 space-y-2 px-4">
                    ...
                    <h2 className="text-sm text-gray-500 dark:text-white/50">{data?.description}</h2>
                </div>
                {/* icons */}
                <div className="mt-4 flex justify-between px-4 pb-4">
                    <div className="flex gap-4 items-center">
                        <button onClick={handleUpVote}><UpVote upVote={data?.upVote}></UpVote></button>

                        <button onClick={handleDwonVote}><DwonVote dwonVote={data?.dwonVote}></DwonVote></button>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={handleComment} className="text-sm font-semibold text-slate-800 dark:text-white/90 px-4 py-2 rounded-full border border-gray-300">Comment</button>
                        <button className="button">Shere</button>
                    </div>
                </div>
                {
                    comment && <div className="relative w-3/4 mx-auto rounded-lg text-center my-5 flex gap-3">
                        <input className="w-full peer rounded-lg border border-sky-600 bg-transparent px-4 py-2 text-sky-600 focus:outline-none" type="text" placeholder="" id="navigate_ui_input_33" name="comment" ref={inputRef} />
