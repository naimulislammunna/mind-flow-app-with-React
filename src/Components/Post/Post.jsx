...
const Post = ({ item }) => {
    const {_id, displayName, photoURL, title, description, tag, time, upVote, dwonVote } = item;

    return (
        <div className="">
            <Link to={`/post/${_id}`}>
                <div className=" mx-auto my-6 max-w-[650px] rounded-lg bg-white shadow-lg dark:bg-[#18181B] border border-gray-500 hover:border-2 hover:border-blue-700 cursor-pointer">
                    <div className="flex items-center justify-between gap-10 px-4 py-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                                ...
                                <p className="text-gray-400">{time?.slice(0, 10)}</p>
                            </div>
                        </div>
                    </div>
                    {/* Post content */}
                    <div className="mt-3 space-y-2 px-4">
                        ...
                        <h2 className="text-sm text-gray-500 dark:text-white/50">{description}<span className="cursor-pointer text-[#3e96d4]">See more</span></h2>
                    </div>
                    {/* icons */}
                    <div className="mt-4 flex justify-between px-4 pb-4">
                        <div className="flex gap-4 items-center">
                            <UpVote upVote={upVote}></UpVote>
                            <DwonVote dwonVote={dwonVote}></DwonVote>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-lg font-semibold text-slate-800 dark:text-white/90">Comment</h2>
                        </div>
                    </div>
                </div>
            </Link>
